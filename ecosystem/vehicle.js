...
function Vehicle(x, y, dx, dy, rad){
	...
	this.acceleration = new JSVector(Math.random(),Math.random());
	this.rad = rad;
...
Vehicle.prototype.seperate = function(Vehicles) {
	...
	for(let i = 0; i < Vehichles.length; i++) {
		let d = new JSVector.getDistance(this.location, Vehicles[i].location);
		
		if ((d>0) && (d < desiredSeperation)) {
			let diff = JSVector.getSubNew(this.location,Vehicles[i].location);
			diff.normalize();
			diff.divide(d);
			steer.add(diff);
			count++;	
...
Vehicle.prototype.align = function(Vehicles) {
	...
	let sum = new JSVector(0,0);
	let count = 0;
	for(let i = 0; Vehicles.length;i++) {
		let d = JSVector.distance(this.position, Vehicles[i].position);
		if((d>0)&&(d < neighborDistance)) {
	...
	if (count > 0) {
		...
		let steer = JSVector.getSubNew(sum,this.velocity);
		steer.limit(this.maxForce);
		return steer;
	} else {
		return new JSVector(0,0);
